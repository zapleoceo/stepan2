...
# Результат «отказ» — единственный, после которого Степан лида НЕ ТРОГАЕТ ВООБЩЕ (решение
# владельца, 10.09.2026). Не пишет первым, не догоняет фолоапами, не будит реактивацией, не
# отправляет в CRM. Гейт переводит такого лида в «менеджер» с выключенным ботом — тем же
# механизмом, что и выигранную сделку.
#
# Кнопка «бот вкл» в Степане отказ НЕ переигрывает: гейт перечитывает CRM не реже раза в
# пять минут, и пока там стоит result_fail, лид будет усыплён снова — с записью «crm hold:
# refused by manager» в журнале треда. Источник истины про отказ — CRM; вернуть Степана
# значит поставить в CRM другой результат контакта.
#
# Одно имя на всех: гейт (стоп-сигнал), сборщики (не писать первым) и отправка в CRM
# (не возвращать в работу) сверяются с этой константой, а не с собственной строкой.
REFUSED_STATUS = "result_fail"
...
POLICIES: dict[str, Policy] = {
    ...
    "result_fail": Policy(
        ...
        goal_en="The conversation with the manager ended in a no. Do not sell. Ask why it "
                ...
                "them alone.",
        # Не инициирует: отказ — стоп-сигнал гейта (REFUSED_STATUS). Текст цели остаётся на
        # случай, если менеджер поставит в CRM другой результат и Степан вернётся в диалог.
        initiates=False,
    ),
    "result_event": Policy(
