...
# Было 4. Раунд r6base 14.08.2026 умер целиком: все десять персон упёрлись в таймаут 900 с, в
# базу не легло ни одного треда, а брокер за всё это время обслужил на филиале 8 ровно два
# вызова sim_lead и встал. Актёр-лида ходит в chat:smart — самую дефицитную линию, — и десять
# персон её выедают; одиночные пробы проходили за 25 с на ход именно потому, что были по одной.
#
# Понижать актёра до chat:fast нельзя: слабый актёр выходит из роли и бросает разговор рано
# (см. комментарий в sim_persona), а он и есть измерительный прибор. Значит платим временем:
# раунд идёт дольше, но доходит.
_CONCURRENCY = 2
...
# 900 с не хватило дважды. В раунде r10 отвалились три персоны — и именно те, что проверяют
# «продолжай разговор» (wants_short_replies, afraid_of_not_keeping_up, polite_soft_no), так что
# выборка перекосилась в лёгкую сторону и раунд стал непригоден для сравнения. В одиночку
# персона проходит семь ходов за три минуты; под нагрузкой очередь к chat:smart растягивает
# ход в разы. Потерянная персона искажает замер сильнее, чем длинный раунд.
_PERSONA_TIMEOUT_S = 1800.0
...
async def score_existing(round_id: str, judge: bool = False) -> dict:
    ...
    И ВТОРОЕ: здесь нет состояния воронки. Пересчёт читает только сообщения, так что
    ready/needs_manager/product пустые у всех, и строка «дошли до готовности» в отчёте
    покажет ноль даже там, где сделка закрылась. В r5 так и вышло: персона
    clicked_smm_ad_wants_to_build_apps дала имя и WhatsApp, а сводка показала ноль. Итоги
    воронки после пересчёта смотреть в базе: lead.stage по тредам sim:suite:<round>:%."""
