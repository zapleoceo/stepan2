...
async def review(  # noqa: PLR0913
    ...
    try:
        raw, meta = await llm.chat(
            ...
            workflow="critic_v3", thread_id=thread_id, branch_id=branch_id)
        if not bill:
        ...
        logger.warning("critic_v3 unavailable branch=%d thread=%d: %s — shipping the draft",
...
def _parse(raw: str) -> Verdict:
    ...
    try:
        ...
        logger.warning("critic_v3: unparseable verdict — shipping the draft")
        return Verdict(sells=True, errored=True)
    if not isinstance(data, dict) or "sells" not in data:
        logger.warning("critic_v3: verdict missing 'sells' — shipping the draft")
        return Verdict(sells=True, errored=True)
    return Verdict(
