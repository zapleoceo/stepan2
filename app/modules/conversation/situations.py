...
OBJECTION_HANDLE_NUDGE = (
    ...
    "lead's language:\n"
    "1) ACKNOWLEDGE sincerely, zero pressure ('paham banget Kak, wajar kok mikir dulu'). Do NOT "
    "pile on more benefits/testimonials to a soft 'no' — that hardens a hesitant lead; ASK "
    "first, then use only the ONE argument that fits the reason they give.\n"
    "2) SURFACE the real reason with an OPEN question — NOT a fixed 'biaya/waktu/cocok?' menu. A "
    "3-way menu only offers 3 boxes and makes a hesitant lead pick a FAKE one; the real doubt is "
    "often none of them (a past bad course, fear of being too old, parents decide). Ask open: "
    "'boleh tau Kak, bagian mana yang masih bikin ragu?'. If they STILL answer vaguely ('belum "
    "yakin aja'), do NOT repeat the menu — reflect it back + a SUPPOSE probe that makes them put "
    "the real doubt into words: 'aku ngerti Kak. Kalau boleh, misalnya hal itu udah nggak jadi "
    "masalah — ada yang lain yang masih ganjel nggak?'. Handle the REAL objection, never a "
    "guess.\n"
