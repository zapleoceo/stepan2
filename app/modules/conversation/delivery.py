...
class ReplyDelivery:
    ...
    async def _sync_lead_fields(self, lead: Lead, thread, decision: Decision) -> None:
        ...
        # Модель переопределяет продукт, унаследованный от рекламы ('ad') или от своей же
        # прошлой догадки ('model'), но никогда — ручной выбор менеджера ('manager').
        #
        # 10.08.2026 замок с 'ad' сняли и в тот же день вернули. Снимали по верному разбору
        # неполного вопроса: вред из треда 4943 шёл через knowledge_context(product_slug),
        # сужавший базу знаний до карточки продукта треда, а той функции в боевом пути больше
        # нет — промт собирает full_knowledge_context() без продукта. Ценовой путь вреда
        # действительно мёртв. Но у замка была вторая работа, и её не взвесили: продукт треда
        # уходит в CRM как «Minat», и по нему менеджер понимает, о чём звонить.
        #
        # Через 33 минуты после снятия тред 2791 (рекламный клик, привязка smm_intensive —
        # верная, Степан весь разговор продавал именно его: Rp 1.882.955, DP 500.000, две
        # недели) был переведён моделью на social_media_bootcamp — другой курс за 750.000 на
        # один день. Ни одной неверной цены в чат не ушло, но менеджеру уехало не то название.
        # Счёт по трём известным случаям: клик прав дважды (4943, 2791), модель права один раз
        # (3163). Один ход догадки слабее клика, так что клик остаётся за старшего.
        #
        # Цель согласия при этом за разговором ИДЁТ — см. _apply_decision: привязка треда
        # отвечает за атрибуцию, а согласие за то, что лежит на столе прямо сейчас. Это
        # разные вопросы, и 3163 закрывается вторым, а не первым.
        #
    ...
    async def _apply_decision(self, lead: Lead, thread, decision: Decision) -> str | None:
        ...
        # Цель, о которой идёт разговор ПРЯМО СЕЙЧАС — не то же самое, что привязка треда.
        # Привязка отвечает за атрибуцию и держится за рекламный клик; согласие спрашивают о
        # том, что лежит на столе. Тред 3163 именно об этом: клик по рекламе ивента, разговор
        # давно о курсе за 13 млн, и «да» ивенту не может считаться «да» курсу.
        #
        # Слаг модели пропускаем через каталог: экстрактор свой сверяет (discovery._one_of), а
        # продающая модель нет, и в досье попадают написания вроде smm-intensive или
        # «SMM Intensive Course». Несуществующий слаг заклинил бы согласие насмерть — записали
        # одно написание, сравнили с другим, и готовность гасится на каждом ходу.
        named = (decision.product_slug or "").strip()
        target = named if named and await self._known_product(named) else thread.product_slug
