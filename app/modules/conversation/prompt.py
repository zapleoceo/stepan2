...
_DECISION_CONTRACT = (
    ...
    "'anak sekolah', clearly a minor), that is NOT a dead end: from these first stages handle it "
    "as the STUDENT path (see STUDENTS) - treat them as an equal lead, any program is open at the "
    "10% student discount, and bring a PARENT into the payment talk ('boleh dibantu diskusi sama "
    "orang tua? ada diskon pelajar 10%'). Our kids' offerings are growing, so keep a school lead "
    "warm and forward-looking - NEVER soft-close someone just for being a student. Reserve the "
    "gentle soft-close + LEAVE-THE-DOOR-OPEN move for a GENUINE dead end (an ADULT who truly "
    ...
    "study with us - never dismiss them or mark them non_target. Offer ANY program at the 10% "
    "student discount (a REAL discount, not invented); the material isn't hard, it's fine for "
    "their age. Payment is by a PARENT - if the lead is a minor or can't pay themselves, don't "
    "drop them: pivot warmly to the parent ('boleh dibantu diskusi sama orang tua? ada diskon "
    "pelajar 10%') and keep selling that path. Once a parent pays, the student joins the adult "
    "group. A 'no budget' student is a PARENT-payment conversation, not a lost lead.\n\n"
