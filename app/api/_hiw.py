  ...
  <details>
    ...
    <div class="body">
      ...
      <details class="l3"><summary>All the way down: the reports</summary><div class="body">
        ...
        <p><b>Real spend, ad by ad.</b> The funnel now sits next to what Meta actually charged. The join is the subtle part: the ad id the private Instagram API hands us is <i>not</i> the Marketing API's ad id — it lives in a different id space and Graph answers "does not exist" for it. The media pk is the bridge: a media's shortcode IS its pk written in Instagram's base64, and Marketing API exposes that shortcode on the creative's permalink. Coverage is honest rather than flattering: 38% of lead-bearing media resolve all the way to an ad. The bridge itself is near-perfect (96.8% of media find their creative) — the loss is downstream: the creative exists in the account but no ad references it, which points at the ads running from a different ad account than the one our token is assigned to. The panel prints that percentage on its own face. It reports cost per <i>our</i> lead and per hand-off — not Meta's headline "cost per conversation", which prices a tap. Meta's own conversation-depth counts (reached message 3, message 5) sit alongside our stages as an independent second opinion, and blocks are surfaced as a spam signal. Files: <code>app/modules/ads/bridge.py</code>, <code>app/adapters/meta_ads.py</code>.</p>
  ...
  <details>
    ...
    <div class="body">
      ...
      <details class="l3"><summary>До дна: звіти</summary><div class="body">
        ...
        <p><b>Реальні витрати, по кожному оголошенню.</b> Воронка тепер стоїть поруч із тим, що Meta справді списала. Тонкість — у стику: ad id, який дає приватний API Instagram, це <i>не</i> ad id з Marketing API — він у іншому просторі ідентифікаторів, і Graph відповідає «об'єкт не існує». Містком є media pk: шорткод медіа — це той самий pk, записаний у base64 алфавітом Instagram, а Marketing API віддає цей шорткод у permalink креатива. Покриття чесне, а не улесливе: 38% медіа з лідами доходять аж до оголошення. Сам місток майже ідеальний (96,8% медіа знаходять свій креатив) — втрата далі: креатив у кабінеті є, але жодне оголошення на нього не посилається, що вказує на рекламу з іншого кабінету, ніж той, до якого прив'язаний наш токен. Панель друкує цей відсоток просто на собі. Вона показує ціну <i>нашого</i> ліда і ліда, що дійшов до передачі, — а не заголовкову «ціну переписки» від Meta, яка оцінює тап. Власні лічильники глибини розмови від Meta (дійшов до 3-го, до 5-го повідомлення) стоять поруч із нашими стадіями як незалежна друга думка, а блокування показані як сигнал спаму. Файли: <code>app/modules/ads/bridge.py</code>, <code>app/adapters/meta_ads.py</code>.</p>
