...
_TR: dict[str, dict[str, str]] = {
    ...
    "inbox.await_settled": {"ru": "Ответ не нужен: передан менеджеру или CRM держит лида",
                            "en": "No reply owed: handed off, or the CRM holds this lead",
                            "id": "Tidak perlu dibalas: sudah diteruskan atau ditahan CRM"},
    ...
    "kb.save_hint": {"ru": "изменения попадут в бота сразу",
                     "en": "changes reach the bot immediately",
                     "id": "perubahan langsung dipakai bot"},
    ...
    "chat.translate_all": {"ru": "Перевести все сообщения", "en": "Translate all messages",
                           "id": "Terjemahkan semua pesan"},
    "chat.tr_result":   {"ru": "Саммари:", "en": "Summary:", "id": "Ringkasan:"},
    "chat.tr_retry":    {"ru": "Перевод не удался — нажмите ещё раз",
                         "en": "Translation failed — tap again",
                         "id": "Terjemahan gagal — ketuk lagi"},
    ...
    "rep.ad_open_manager": {"ru": "Открыть в рекламном кабинете",
                            "en": "Open in Ads Manager",
                            "id": "Buka di Ads Manager"},
    "rep.ad_open_fb":   {"ru": "Открыть в библиотеке рекламы",
                         "en": "Open in the Ad Library",
                         "id": "Buka di Ad Library"},
    ...
    "ch.connect_fb": {"ru": "Подключить Facebook",
                      "en": "Connect Facebook",
                      "id": "Hubungkan Facebook"},
    "ch.connect_fb_hint": {
        "ru": "Откроется окно Meta: выберите Страницу и разрешите доступ. Токен придёт сам.",
        "en": "Opens Meta's window: pick the Page and allow access. The token arrives by itself.",
        "id": "Membuka jendela Meta: pilih Halaman dan izinkan akses. Token datang sendiri."},
    "ch.connect_manual": {"ru": "Или вставить токен вручную",
                          "en": "Or paste a token manually",
                          "id": "Atau tempel token manual"},
    ...
    "ch.wa_ro":    {"ru": "Только чтение",
                    "en": "Read-only",
                    "id": "Hanya baca"},
    "ch.wa_ro_hint": {
        "ru": "Номер менеджера: читаем переписку для аналитики, ничего не отправляем.",
        "en": "A manager's number: we read the chats for analytics and never send.",
        "id": "Nomor manajer: kami membaca obrolan untuk analitik dan tidak pernah mengirim."},
    "ch.wa_phone": {"ru": "Номер WhatsApp", "en": "WhatsApp number", "id": "Nomor WhatsApp"},
    "ch.wa_pair":  {"ru": "Привязать телефон", "en": "Link phone", "id": "Hubungkan ponsel"},
    "ch.wa_pairing_for": {"ru": "Привязка номера", "en": "Linking", "id": "Menghubungkan"},
    "ch.wa_step1": {
        "ru": "На телефоне: WhatsApp → Настройки → Связанные устройства",
        "en": "On the phone: WhatsApp → Settings → Linked devices",
        "id": "Di ponsel: WhatsApp → Setelan → Perangkat tertaut"},
    "ch.wa_step2": {
        "ru": "«Привязка устройства» и отсканировать этот код",
        "en": "\"Link a device\", then scan this code",
        "id": "\"Tautkan perangkat\", lalu pindai kode ini"},
    "ch.wa_step3": {
        "ru": "Код живёт около минуты — если истёк, нажмите «Новый код»",
        "en": "The code lasts about a minute — press \"New code\" if it expires",
        "id": "Kode berlaku sekitar satu menit — tekan \"Kode baru\" jika kedaluwarsa"},
    "ch.wa_qr_new": {"ru": "Новый код", "en": "New code", "id": "Kode baru"},
    "ch.wa_qr_none": {
        "ru": "Код не пришёл. Нажмите «Новый код».",
        "en": "No code came back. Press \"New code\".",
        "id": "Kode tidak diterima. Tekan \"Kode baru\"."},
    "ch.wa_off":   {"ru": "WhatsApp не настроен на сервере (EVOLUTION_URL)",
                    "en": "WhatsApp is not configured on the server (EVOLUTION_URL)",
                    "id": "WhatsApp belum dikonfigurasi di server (EVOLUTION_URL)"},
    "ch.wa_need_phone": {"ru": "Введите номер телефона",
                         "en": "Enter the phone number",
                         "id": "Masukkan nomor telepon"},
    "ch.cancel":   {"ru": "Отмена", "en": "Cancel", "id": "Batal"},
    ...
    # Страница «Стратегия». Схема собирается из живого кода, поэтому подписи здесь — про
    # смысл шага, а числа подставляются из модулей и в переводе не участвуют.
    "stg.title": {"ru": "Как Степан решает на каждом ходу",
                  "en": "How Stepan decides on every turn",
                  "id": "Bagaimana Stepan memutuskan tiap giliran"},
    "stg.sub": {
        "ru": "Схема собрана из кода: пороги и расписания читаются из тех же модулей, что и "
              "воркер, поэтому разойтись с поведением они не могут.",
    ...
    "stg.phases": {
        "ru": "Ход НЕ линеен: это три фазы в разных процессах. Отбор — крон раз в минуту, "
              "генерация — отдельная задача, отправка — другой крон каждые 10 секунд. Между "
              "фазами состояние успевает измениться, поэтому блокировка проверяется дважды.",
        "en": "A turn is NOT linear: three phases in different processes. Selection runs "
              "once a minute, generation is its own job, sending is another cron every 10 s. "
              "State can change in between — which is why blocking is checked twice.",
        "id": "Satu giliran TIDAK linear: tiga fase di proses berbeda."},
    "stg.ph.pick": {"ru": "Фаза 1. Отбор: кому вообще отвечаем",
                    "en": "Phase 1. Selection: who gets an answer at all",
                    "id": "Fase 1. Seleksi"},
    "stg.ph.pick.sub": {"ru": "Крон раз в минуту. Ничего не генерирует — только находит треды.",
                        "en": "A cron once a minute. Generates nothing — it only finds threads.",
                        "id": "Cron tiap menit; hanya mencari thread."},
    "stg.ph.pick.end": {"ru": "Тред уходит в очередь на генерацию",
                        "en": "Thread queued for generation", "id": "Thread masuk antrean"},
    "stg.ph.gen": {"ru": "Фаза 2. Генерация: что именно отвечаем",
                   "en": "Phase 2. Generation: what exactly we answer",
                   "id": "Fase 2. Generasi"},
    "stg.ph.gen.sub": {
        "ru": "Отдельная задача на тред, со своим таймаутом. Здесь тратятся деньги брокера.",
        "en": "Its own job per thread, with its own timeout. This is where broker money goes.",
        "id": "Tugas sendiri per thread."},
    "stg.ph.gen.start": {"ru": "Задача взяла тред", "en": "The job picks the thread up",
                         "id": "Tugas mengambil thread"},
    "stg.ph.gen.end": {"ru": "Ответ разбит на пузыри и поставлен в очередь",
                       "en": "Reply split into bubbles and queued",
                       "id": "Balasan masuk antrean"},
    "stg.ph.send": {"ru": "Фаза 3. Отправка: дойдёт ли написанное",
                    "en": "Phase 3. Sending: will the written message go out",
                    "id": "Fase 3. Pengiriman"},
    "stg.ph.send.sub": {
        "ru": "Крон каждые 10 секунд. Здесь стоит гейт CRM — сообщение к этому моменту уже "
              "сочинено и оплачено, поэтому «стоп» от CRM стоит нам вызова брокера.",
        "en": "A cron every 10 s. The CRM gate lives here — by now the message is written and "
              "paid for, so a CRM stop costs us a broker call.",
        "id": "Cron tiap 10 detik; gerbang CRM ada di sini."},
    "stg.ph.send.start": {"ru": "Строка в очереди", "en": "A queued row",
                          "id": "Baris di antrean"},
    "stg.q.stale": {"ru": "Сообщение слишком старое?", "en": "Message too old?",
                    "id": "Pesan terlalu lama?"},
    "stg.t.stale": {"ru": "Не отвечаем: отвечать через сутки хуже, чем не отвечать",
                    "en": "Skipped: a day-late answer is worse than none",
                    "id": "Dilewati: terlalu terlambat"},
    "stg.q.budget": {"ru": "Дневной бюджет на модель исчерпан?", "en": "Daily LLM budget spent?",
                     "id": "Anggaran harian habis?"},
    "stg.t.budget": {"ru": "Ждём до завтра, тред остаётся в очереди",
                     "en": "Wait for tomorrow; the thread stays queued",
                     "id": "Menunggu besok"},
    "stg.q.rsvp": {"ru": "Запись на мероприятие?", "en": "An event RSVP?",
                   "id": "Pendaftaran acara?"},
    "stg.t.rsvp": {"ru": "Уведомляем, но НЕ передаём: бот продолжает вести разговор",
                   "en": "Notify but do NOT hand off: the bot keeps talking",
                   "id": "Beri tahu, tapi jangan serahkan"},
    "stg.q.discovery": {"ru": "Боль ещё не выявлена?", "en": "Pain not captured yet?",
                        "id": "Kebutuhan belum tergali?"},
    "stg.t.discovery": {"ru": "Не презентуем: сначала выяснить, зачем человек пришёл",
                        "en": "Do not present yet: find out why they came first",
                        "id": "Jangan presentasi dulu"},
    "stg.q.paused": {"ru": "Отправка на паузе?", "en": "Sending paused?",
                     "id": "Pengiriman dijeda?"},
    "stg.t.paused": {"ru": "Очередь копится: обычно так гасят soft-block от Instagram",
                     "en": "The queue builds up: this is how an Instagram soft-block is held",
                     "id": "Antrean menumpuk"},
    "stg.q.blocked2": {"ru": "Лид заблокирован (проверка вторая)?",
                       "en": "Lead blocked (second check)?",
                       "id": "Lead diblokir (cek kedua)?"},
    "stg.t.blocked2": {"ru": "Строку выбрасываем: заблокировать могли уже после генерации",
                       "en": "Row dropped: the block may have landed after generation",
                       "id": "Baris dibuang"},
    "stg.q.quiet": {"ru": "Тихие часы и это фолоу-ап?", "en": "Quiet hours and a follow-up?",
                    "id": "Jam tenang dan follow-up?"},
    "stg.t.quiet": {"ru": "Держим до утра. Реактивация исключена намеренно, ответы не глушатся",
                    "en": "Held until morning. Reactivation is deliberately exempt; replies go",
                    "id": "Ditahan sampai pagi"},
    "stg.q.caps": {"ru": "Часовой или суточный кап выбран?", "en": "Hourly or daily cap hit?",
                   "id": "Batas jam/hari tercapai?"},
    "stg.t.caps": {"ru": "Остаётся в очереди до следующего окна — это антибан",
                   "en": "Stays queued until the next window — this is the ban guard",
                   "id": "Tetap di antrean"},
    "stg.q.window": {"ru": "Окно Meta закрыто (24 часа)?", "en": "Meta 24h window closed?",
                     "id": "Jendela Meta tertutup?"},
    "stg.t.window": {"ru": "Не пытаемся: Graph такое отклоняет, помечаем skipped",
                     "en": "No attempt: Graph rejects it; marked skipped",
                     "id": "Tidak dicoba"},
    "stg.q.soft": {"ru": "Instagram ответил soft-block?", "en": "Instagram answered soft-block?",
                   "id": "Instagram balas soft-block?"},
    "stg.t.soft": {"ru": "Отложить и повторить позже; долбить повторами — углублять блок",
                   "en": "Back off and retry later; hammering deepens the block",
                   "id": "Tunda dan coba lagi"},
    "stg.crm.title": {"ru": "Когда и что уезжает в CRM",
                      "en": "When and what goes to the CRM",
                      "id": "Kapan dan apa yang dikirim ke CRM"},
    "stg.crm.sub": {
        "ru": "Три пути и одно правило: одно состояние уезжает один раз. Смена состояния — "
              "новое событие и уезжает, повтор того же — нет.",
        "en": "Three paths, one rule: a state is pushed once. A change of state is a new "
              "event and goes; a repeat of the same one does not.",
        "id": "Tiga jalur, satu aturan: satu keadaan dikirim sekali."},
    "stg.c.start": {"ru": "Появился повод передать", "en": "Something to push",
                    "id": "Ada alasan mengirim"},
    "stg.c.end": {"ru": "Событие ушло в CRM", "en": "Event lands in the CRM",
                  "id": "Peristiwa masuk CRM"},
    "stg.c.q.phone": {"ru": "Телефона нет?", "en": "No phone?", "id": "Tidak ada telepon?"},
    "stg.c.t.phone": {"ru": "Не передаём ни одним путём: CRM связывает по номеру",
                      "en": "No path pushes: the CRM joins by phone number",
                      "id": "Tidak dikirim: CRM memakai nomor"},
    "stg.c.q.blocked": {"ru": "Лид заблокирован?", "en": "Lead blocked?",
                        "id": "Lead diblokir?"},
    "stg.c.t.blocked": {"ru": "Не передаём: спам не должен становиться задачей менеджера",
                        "en": "Not pushed: spam must not become a manager's task",
                        "id": "Tidak dikirim: spam bukan tugas manajer"},
    "stg.c.q.off": {"ru": "Запись в CRM выключена?", "en": "CRM writeback off?",
                    "id": "Penulisan CRM mati?"},
    "stg.c.t.off": {"ru": "Ничего не уходит, лид ждёт включения",
                    "en": "Nothing leaves; the lead waits until it is on",
                    "id": "Tidak ada yang dikirim"},
    "stg.c.q.flip": {"ru": "Прямо сейчас стал READY или MANAGER?",
                     "en": "Just became READY or MANAGER?",
                     "id": "Baru jadi READY atau MANAGER?"},
    "stg.c.t.flip": {"ru": "Мгновенно, сразу после коммита ответа: wait_call и «передан команде»",
                     "en": "Instantly after the reply commits: wait_call, handed to the team",
                     "id": "Segera setelah balasan tersimpan"},
    "stg.c.q.same": {"ru": "Это же состояние уже отправляли?",
                     "en": "Was this same state already pushed?",
                     "id": "Keadaan sama sudah dikirim?"},
    "stg.c.t.same": {"ru": "Молчим: повтор засоряет ленту менеджера ложным касанием",
                     "en": "Skip: a repeat litters the manager's timeline with a fake touch",
                     "id": "Lewati: pengulangan mengotori linimasa"},
    "stg.c.q.human": {"ru": "Лид уже у человека (ready, manager, handed_off)?",
                      "en": "Already human-led (ready, manager, handed_off)?",
                      "id": "Sudah dipegang manusia?"},
    "stg.c.t.human": {"ru": "Подметатель хендоффов: wait_call и «свяжитесь немедленно»",
                      "en": "Hand-off sweep: wait_call, contact them now",
                      "id": "Sapuan serah terima: hubungi segera"},
    "stg.c.q.old": {"ru": "Передача старше окна?", "en": "Hand-off older than the window?",
                    "id": "Lebih tua dari jendela?"},
    "stg.c.t.old": {"ru": "Автоматически не уедет никогда — счётчик пишет это в лог",
                    "en": "Never picked up automatically — the counter logs it",
                    "id": "Tidak akan terkirim otomatis"},
    "stg.when.title": {"ru": "Три пути передачи", "en": "The three push paths",
                       "id": "Tiga jalur pengiriman"},
    "stg.when.path": {"ru": "Путь", "en": "Path", "id": "Jalur"},
    "stg.when.when": {"ru": "Когда", "en": "When", "id": "Kapan"},
    "stg.when.what": {"ru": "Что уходит", "en": "What is sent", "id": "Yang dikirim"},
    "stg.when.mark": {"ru": "Маркер", "en": "Marker", "id": "Penanda"},
    "stg.w.flip": {"ru": "Мгновенный", "en": "Instant", "id": "Instan"},
    "stg.w.flip.when": {"ru": "в момент передачи человеку, если телефон уже есть",
                        "en": "at hand-off, when a phone is already on file",
                        "id": "saat serah terima"},
    "stg.w.flip.what": {"ru": "wait_call + контекст переписки",
                        "en": "wait_call + the chat context", "id": "wait_call + konteks"},
    "stg.w.warm": {"ru": "Тёплый сгон", "en": "Warm sweep", "id": "Sapuan hangat"},
    "stg.w.warm.when": {"ru": "раз в час, лиды в работе с телефоном",
                        "en": "hourly, in-progress leads with a phone",
                        "id": "tiap jam, lead aktif"},
    "stg.w.warm.what": {"ru": "тип по состоянию: wait_call, thinking или reject",
                        "en": "type by state: wait_call, thinking or reject",
                        "id": "tipe sesuai keadaan"},
    "stg.w.sweep": {"ru": "Подметатель", "en": "Hand-off sweep", "id": "Sapuan"},
    "stg.w.sweep.when": {"ru": "тот же прогон; телефон пришёл ПОСЛЕ передачи",
                         "en": "same run; the phone arrived AFTER the hand-off",
                         "id": "telepon datang setelah serah terima"},
    "stg.w.sweep.what": {"ru": "wait_call + «уже передан команде»",
                         "en": "wait_call + already handed to the team",
                         "id": "wait_call + sudah diserahkan"},
    "stg.q.start": {"ru": "Лид написал", "en": "The lead wrote", "id": "Lead menulis"},
    "stg.q.send": {"ru": "Отправляем ответ", "en": "Send the reply", "id": "Kirim balasan"},
    "stg.q.blocked": {"ru": "Лид заблокирован?", "en": "Is the lead blocked?",
                      "id": "Lead diblokir?"},
    "stg.t.blocked": {"ru": "Игнорируем целиком: ни ответа, ни алерта в Telegram",
                      "en": "Ignored entirely: no reply, no Telegram alert",
                      "id": "Diabaikan sepenuhnya"},
    "stg.q.channel": {"ru": "Канал выключен?", "en": "Is the channel switched off?",
                      "id": "Kanal dimatikan?"},
    "stg.t.channel": {"ru": "Не генерируем: отправить некуда, строки ретируются",
                      "en": "Nothing generated: undeliverable, rows retired",
                      "id": "Tidak dibuat: tidak terkirim"},
    "stg.q.agent": {"ru": "Бота выключил человек?", "en": "Did a human switch the bot off?",
                    "id": "Bot dimatikan manusia?"},
    "stg.t.agent": {"ru": "Молчим: тред ведёт менеджер", "en": "Silent: a manager owns it",
                    "id": "Diam: manajer memegangnya"},
    "stg.q.silent": {"ru": "Стадия из молчащих?", "en": "A bot-silent stage?",
                     "id": "Tahap bot diam?"},
    "stg.t.silent": {"ru": "Молчим: ready, handed_off, dormant",
                     "en": "Silent: ready, handed_off, dormant",
                     "id": "Diam: ready, handed_off, dormant"},
    "stg.q.pending": {"ru": "Ответ уже в очереди?", "en": "A reply already queued?",
                      "id": "Balasan sudah antre?"},
    "stg.t.pending": {"ru": "Ждём: второй раз не генерируем, иначе уйдёт дубль",
                      "en": "Wait: no second generation, it would ship a duplicate",
                      "id": "Tunggu: hindari duplikat"},
    "stg.q.bye": {"ru": "Оба попрощались?", "en": "Both sides said goodbye?",
                  "id": "Keduanya berpamitan?"},
    "stg.t.bye": {"ru": "Молчим: разговор закончен, лид написал только пустое",
                  "en": "Silent: the conversation is over",
                  "id": "Diam: percakapan selesai"},
    "stg.q.media": {"ru": "Ждём распознавания медиа?", "en": "Waiting on media?",
                    "id": "Menunggu media?"},
    "stg.t.media": {"ru": "Ждём: сначала расшифровка голоса или картинки",
                    "en": "Wait: transcription first", "id": "Tunggu: transkripsi dulu"},
    "stg.q.won": {"ru": "CRM: сделка закрыта или оплачена?", "en": "CRM: deal closed or paid?",
                  "id": "CRM: transaksi selesai?"},
    "stg.t.won": {"ru": "Молчим совсем: болтливый бот после договора только отговорит",
                  "en": "Fully silent: a chatty bot after a contract only undoes it",
                  "id": "Diam total setelah kontrak"},
    "stg.q.owner": {"ru": "CRM: менеджер ведёт или запланирован звонок?",
                    "en": "CRM: manager owns it or a call is scheduled?",
                    "id": "CRM: manajer memegang / panggilan dijadwalkan?"},
    "stg.t.owner": {"ru": "Отвечаем, но первыми не пишем — и агента не выключаем",
                    "en": "Answer, never start — and the agent stays on",
                    "id": "Menjawab, tidak memulai"},
    "stg.q.guard": {"ru": "Проверка фактов не прошла?", "en": "Did the fact guard fail?",
                    "id": "Pemeriksaan fakta gagal?"},
    "stg.t.guard": {"ru": "Заглушка вместо выдумки и алерт менеджеру",
                    "en": "A hold line instead of an invention, plus an alert",
                    "id": "Kalimat tunda, bukan karangan"},
    "stg.q.ready": {"ru": "Готов и есть телефон?", "en": "Ready and has a phone?",
                    "id": "Siap dan ada telepon?"},
    "stg.t.ready": {"ru": "Стадия READY и мгновенная передача в CRM",
                    "en": "Stage READY and an instant CRM push",
                    "id": "Tahap READY dan kirim ke CRM"},
    "stg.q.manager": {"ru": "Нужен менеджер?", "en": "Needs a manager?",
                      "id": "Perlu manajer?"},
    "stg.t.manager": {"ru": "Стадия MANAGER, алерт и передача, если телефон есть",
                      "en": "Stage MANAGER, alert, and a push when a phone exists",
                      "id": "Tahap MANAGER, peringatan"},
    "stg.in": {"ru": "Сообщение дошло до нас", "en": "The message reaches us",
               "id": "Pesan sampai ke kami"},
    "stg.in.n": {"ru": "полный опрос раз в 2 мин, быстрая полоса между",
                 "en": "full poll every 2 min, fast lane in between",
                 "id": "polling penuh tiap 2 mnt"},
    "stg.pick": {"ru": "Отбор на ответ", "en": "Picked for a reply", "id": "Dipilih dibalas"},
    "stg.pick.n": {"ru": "лид написал последним, тред за ботом",
                   "en": "lead spoke last, thread owned by the bot",
                   "id": "lead menulis terakhir"},
    "stg.gen": {"ru": "Сборка промта и ответ", "en": "Prompt and generation",
                "id": "Prompt dan generasi"},
    "stg.gen.n": {"ru": "знания, персона, досье, заметка, состояние CRM",
                  "en": "knowledge, persona, dossier, note, CRM state",
                  "id": "pengetahuan, persona, dosir, status CRM"},
    "stg.guard": {"ru": "Проверки", "en": "Guards", "id": "Pemeriksaan"},
    "stg.guard.n": {"ru": "цены, ссылки, обещания — не прошло, значит человек",
                    "en": "prices, links, promises — fails closed to a human",
                    "id": "harga, tautan, janji — gagal ke manusia"},
    "stg.queue": {"ru": "Очередь и отправка", "en": "Queue and send", "id": "Antrean dan kirim"},
    "stg.queue.n": {"ru": "пузыри, паузы, капы, отправщик раз в 10 сек",
                    "en": "bubbles, pauses, caps, sender every 10 s",
                    "id": "gelembung, jeda, batas"},
    "stg.stage": {"ru": "Стадия и передача", "en": "Stage and hand-off",
                  "id": "Tahap dan serah terima"},
    "stg.stage.n": {"ru": "готов + телефон → передача сразу",
                    "en": "ready + phone → instant hand-off",
                    "id": "siap + telepon → serah terima"},
    "stg.crm": {"ru": "Ответ CRM", "en": "What the CRM says back", "id": "Balasan CRM"},
    "stg.crm.n": {"ru": "результат менеджера задаёт следующий ход",
                  "en": "the manager's outcome sets the next move",
                  "id": "hasil manajer menentukan langkah"},
    "stg.x.block": {"ru": "Молчим", "en": "Silent", "id": "Diam"},
    "stg.x.bye": {"ru": "Молчим", "en": "Silent", "id": "Diam"},
    "stg.x.hold": {"ru": "Держим", "en": "Held", "id": "Ditahan"},
    "stg.x.quiet": {"ru": "Ждём", "en": "Wait", "id": "Menunggu"},
    "stg.k.window": {"ru": "окно подметателя, дней", "en": "sweep window, days",
                     "id": "jendela sapuan, hari"},
    "stg.k.batch": {"ru": "пачка в CRM", "en": "CRM batch", "id": "batch CRM"},
    "stg.k.react": {"ru": "реактивация за прогон", "en": "reactivation per run",
                    "id": "reaktivasi per proses"},
    "stg.k.cap": {"ru": "follow-through за прогон", "en": "follow-through per run",
                  "id": "follow-through per proses"},
    "stg.k.cooldown": {"ru": "кулдаун, дней", "en": "cooldown, days", "id": "jeda, hari"},
    "stg.k.quiet": {"ru": "тишина после нашего, ч", "en": "quiet after ours, h",
                    "id": "hening setelah kami, jam"},
    "stg.k.work": {"ru": "рабочие часы", "en": "working hours", "id": "jam kerja"},
    "stg.pol.title": {"ru": "Что Степан делает после менеджера",
                      "en": "What Stepan does after the manager",
                      "id": "Apa yang Stepan lakukan setelah manajer"},
    "stg.pol.sub": {
        "ru": "Пять статусов — те, которыми филиал реально пользуется. Доли по 2000 контактам "
              "за 30 дней. Договор Степан не оформляет: это зона менеджера.",
        "en": "The five statuses the branch actually uses; shares over 2000 contacts in 30 "
              "days. Stepan never files a contract — that is the manager's job.",
        "id": "Lima status yang benar-benar dipakai cabang."},
    "stg.pol.status": {"ru": "Статус в CRM", "en": "CRM status", "id": "Status CRM"},
    "stg.pol.share": {"ru": "Доля", "en": "Share", "id": "Porsi"},
    "stg.pol.means": {"ru": "Что значит", "en": "Means", "id": "Artinya"},
    "stg.pol.does": {"ru": "Что делает Степан", "en": "What Stepan does",
                     "id": "Tindakan Stepan"},
    "stg.pol.wait_call.m": {"ru": "менеджер ждёт созвона", "en": "manager awaits the call",
                            "id": "manajer menunggu panggilan"},
    "stg.pol.wait_call.d": {"ru": "сами не пишем, на входящее отвечаем",
                            "en": "never start, always answer",
                            "id": "tidak memulai, tetap menjawab"},
    "stg.pol.result_think.m": {"ru": "взял паузу", "en": "taking time", "id": "menimbang"},
    "stg.pol.result_think.d": {"ru": "спросить срок и назвать день; иначе через месяц",
                               "en": "ask for a date; a month if none given",
                               "id": "tanya tanggal; sebulan bila tidak ada"},
    "stg.pol.result_next_enrollment.m": {"ru": "нужен следующий набор", "en": "next intake",
                                         "id": "angkatan berikutnya"},
    "stg.pol.result_next_enrollment.d": {"ru": "выяснить какой и отложить до старта",
                                         "en": "find out which, defer to its start",
                                         "id": "cari tahu yang mana, tunda"},
    "stg.pol.result_fail.m": {"ru": "отказ", "en": "refusal", "id": "penolakan"},
    "stg.pol.result_fail.d": {"ru": "разобрать причину, один вопрос; молчит — попрощаться",
                              "en": "one honest question; silence ends it warmly",
                              "id": "satu pertanyaan jujur"},
    "stg.pol.result_event.m": {"ru": "придёт на мероприятие", "en": "coming to an event",
                               "id": "akan datang ke acara"},
    "stg.pol.result_event.d": {"ru": "ничего не делаем", "en": "nothing to do",
                               "id": "tidak ada tindakan"},
    "stg.sil.title": {"ru": "Когда Степан молчит", "en": "When Stepan stays quiet",
                      "id": "Kapan Stepan diam"},
    "stg.sil.sub": {
        "ru": "Отвечать и писать первым — разные вещи. До 30.07 это было одним действием, и "
              "звонок менеджера глушил ответы на трое суток.",
        "en": "Answering and starting are different things. Until 30 July they were one, and "
              "a manager's call silenced replies for three days.",
        "id": "Menjawab dan memulai adalah dua hal berbeda."},
    "stg.sil.reason": {"ru": "Причина", "en": "Reason", "id": "Alasan"},
    "stg.sil.answers": {"ru": "Отвечает на входящее", "en": "Answers incoming",
                        "id": "Menjawab masuk"},
    "stg.sil.starts": {"ru": "Пишет первым", "en": "Starts a chat", "id": "Memulai chat"},
    "stg.s.blocked": {"ru": "Лид заблокирован", "en": "Lead is blocked",
                      "id": "Lead diblokir"},
    "stg.s.manual": {"ru": "Бота выключил человек", "en": "A human switched the bot off",
                     "id": "Bot dimatikan manusia"},
    "stg.s.won": {"ru": "Сделка закрыта или оплачена", "en": "Deal closed or paid",
                  "id": "Transaksi selesai"},
    "stg.s.manager": {"ru": "Менеджер ведёт лида", "en": "A manager owns the lead",
                      "id": "Manajer memegang lead"},
    "stg.s.planned": {"ru": "Запланирован звонок", "en": "A call is scheduled",
                      "id": "Panggilan dijadwalkan"},
    "stg.s.bye": {"ru": "Оба попрощались", "en": "Both sides said goodbye",
                  "id": "Keduanya berpamitan"},
    "stg.s.quiet": {"ru": "Тихие часы", "en": "Quiet hours", "id": "Jam tenang"},
    "stg.s.channel": {"ru": "Канал выключен", "en": "Channel switched off",
                      "id": "Kanal dimatikan"},
