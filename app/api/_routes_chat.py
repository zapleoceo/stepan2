...
async def chat_page(
    ...
    filter (stage/lead_type/audience/ad_id/grp/awaiting/kind) rides along in the query so a
    full reload keeps the filtered thread list instead of reverting to the whole inbox."""
    ...
    resp = HTMLResponse(app_shell(
        ...
        ad_id=ad_id.strip(), grp=grp.strip(), awaiting=awaiting.strip(), kind=kind.strip()))
