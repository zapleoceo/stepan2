...
class ChannelKind(StrEnum):
    ...
    # Переписка, доступная ТОЛЬКО через sender CRM: входящие приносит колбек, ответ уходит
    # их инструментом. Отдельный вид, а не режим WHATSAPP: тот работает через Evolution API
    # на нашем сервере, здесь же чужой транспорт, чужие идентификаторы и отправка, которая
    # не подтверждает доставку.
    #
    # ОДИН вид на все их мессенджеры, а не по одному на каждый. На стороне sender у филиала
    # включены whats-app, telegram, viber и smsviber; транспорт, учётные данные, адресация
    # ответа и правила дедупликации у них общие, а различает их одно поле в полезной нагрузке.
    # Заводить вид на каждый значило бы копировать коннектор ради этого поля — и заставлять
    # оператора подключать четыре канала там, где связь одна.
    CRM_SENDER = "crm_sender"
