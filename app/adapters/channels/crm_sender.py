...
def strip_crm_markup(text: str | None) -> str:
    ...
    Снимается ТОЛЬКО то, что добавляет CRM: карточка превью, ссылка-обёртка и перенос
    строки. Произвольные теги не трогаем, и это осознанно — у нас есть живое сообщение,
    где Степан учит лида тегам («cukup kuasai tag dasar kayak `<html>`, `<head>`…»), и
    подметание всего подряд стёрло бы ровно то, ради чего оно написано. Незнакомая
    разметка останется видимой в чате — это заметят и починят, в отличие от тихой потери.
...
class CrmSenderAdapter:
    ...
    async def fetch_inbound(self) -> list[InboundMessage]:
        """Необработанное из своей таблицы, помеченное как разобранное.

        ОБЕ стороны разговора, как и у остальных коннекторов. Направление едет с сообщением,
        и ingest сам разводит: входящее — ход лида, исходящее — `_store_outgoing`, который
        двигает `last_out_at` и репликой лида его не считает. Раньше здесь стоял фильтр
        `direction == 'in'` из опасения «Степан ответит менеджеру», но опасение снимает не
        фильтр, а разбор направления ниже по течению: отфильтрованные строки просто оставались
        в таблице навсегда, а тред продолжал выглядеть неотвеченным — то есть фильтр давал
        ровно тот вред, от которого защищал. Исходящее менеджера — единственный признак, что
        человек уже в разговоре.
        ...
        for row in rows:
            ...
            out.append(InboundMessage(
                ...
                # Имя чужое только у входящего. У исходящего `from_name` — это менеджер, и
                # записать его на карточку значило бы переименовать лида в сотрудника.
                sender_name=row.from_name if row.direction == "in" else None,
            ))
