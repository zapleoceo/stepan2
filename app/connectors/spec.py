...
class ConnectorSpec:
    ...
    # Живут ли разговоры этого коннектора УЖЕ в CRM.
    #
    # Отправка «тёплого лида в CRM» отвечает на вопрос «этого человека в CRM ещё нет —
    # сообщить». Для коннектора, который сам читает переписку ИЗ CRM (sender), ответ известен
    # заранее: человек там есть, менеджер уже с ним говорит. Отправить его обратно значит
    # вернуть в работу карточку, которую менеджер закрыл — 11–12.08.2026 так уехали 535 лидов
    # из загруженной истории, 129 из них уже с отказом. Факт объявлен здесь, чтобы отбор на
    # отправку спрашивал реестр, а не отрастил `if kind ==`.
    crm_native: bool = False
