...
def test_earlier_blocks_win_because_order_is_priority() -> None:
    ...
    Размеры считаются ОТ бюджета, а не пишутся числом: с константой 40000 тест молча
    перестал проверять приоритет в тот день, когда потолок подняли до 122k — три блока
    просто уместились, и «хвост режется» больше ничем не подтверждалось.
    """
    # По 40% бюджета: двое умещаются, трое — нет, при любом потолке.
    size = _FREE_CTX_CHAR_BUDGET * 2 // 5
    persona = "P" * size
    policy = "L" * size
    card = "C" * size
