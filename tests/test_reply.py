...
def test_only_one_rule_claims_the_start_of_the_message() -> None:
    ...
    # Якорь менялся дважды вслед за формулировкой правила о длине; проверяем не текст, а
    # место — разрешение должно оставаться разрешением, а не переезжать в жёсткие правила.
    assert "Slow down" in allowed and "Emoji" in allowed
