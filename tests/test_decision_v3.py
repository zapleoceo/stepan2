...
from app.modules.conversation.decision_v3 import DecisionV3, parse_decision_v3
...
def test_tolerates_markdown_fences() -> None:
    assert parse_decision_v3(f"```json\n{_raw()}\n```").reply == "halo kak"
...
def test_a_broken_contract_raises_rather_than_returning_a_bad_reply(bad: str) -> None:
    with pytest.raises(ValueError, match="decision"):
        parse_decision_v3(bad)


def test_an_unknown_move_degrades_instead_of_aborting_the_turn() -> None:
    assert parse_decision_v3(_raw(move="upsell_hard")).move == "give_value"
    assert parse_decision_v3(_raw(move=None)).move == "give_value"


def test_an_unknown_stage_falls_back_to_an_active_one() -> None:
    assert parse_decision_v3(_raw(stage="greeting")).stage is Stage.QUALIFYING


def test_a_malformed_dossier_costs_the_learning_not_the_reply() -> None:
    d = parse_decision_v3(_raw(dossier="oops"))
    assert d.reply == "halo kak"
    assert d.dossier == LeadDossier()


def test_objections_parse_with_status_and_as_bare_strings() -> None:
    d = parse_decision_v3(_raw(dossier={"objections": [
...
def test_reply_language_only_survives_when_it_looks_like_a_code() -> None:
    assert parse_decision_v3(_raw(reply_language="ru")).reply_language == "ru"
    assert parse_decision_v3(_raw(reply_language="bahasa indonesia")).reply_language is None
...
def test_legacy_fields_come_from_the_merged_dossier_not_just_this_turn() -> None:
    d = DecisionV3(reply="ok", move="close", stage=Stage.PRESENTING)
...
def test_a_blunt_refusal_maps_to_the_legacy_hard_stop() -> None:
    ...
    legacy = DecisionV3(reply="ok", move="accept_refusal", stage=Stage.DORMANT).to_legacy(
...
def test_a_soft_refusal_does_not_stop_anything() -> None:
    legacy = DecisionV3(reply="ok", move="accept_refusal", stage=Stage.NURTURING).to_legacy(
...
def test_lead_type_is_derived_from_state_rather_than_asked_for_separately() -> None:
    def kind(**kw) -> str | None:  # noqa: ANN003
        return DecisionV3(reply="x", move="give_value", stage=Stage.QUALIFYING).to_legacy(
...
def test_escalation_reason_reaches_both_legacy_fields() -> None:
    legacy = DecisionV3(reply="ok", move="escalate_human", stage=Stage.MANAGER,
