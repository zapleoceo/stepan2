...
to v2 rather than to a code path that doesn't exist."""
...
def test_default_engine_is_v2() -> None:
    """A branch that never set the key keeps the existing pipeline."""
    assert _parse({}).reply_engine == "v2"
...
def test_unrecognised_engine_falls_back_to_v2(bad: str) -> None:
    """A typo or a stale row from a removed option must not disable replies."""
    assert _parse({"reply_engine": bad}).reply_engine == "v2"
