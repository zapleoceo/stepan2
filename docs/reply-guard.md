...
## Pipeline (`app/modules/conversation/guard.py`, wired in `ReplyService._guard_reply`)
...
2. **Selective LLM verify (`chat:fast`, `kind='guard'`):** only when the reply looks risky
   (offer/resource/access/link keywords) a cheap model lists claims unsupported by the KB
   context. Not run on ordinary replies → bounded cost.
