...
| Тестирование сценариев | пройдено по обоим |
| Домены + redirect URI | исправлены 03.08 (см. следующий раздел) |
| Режим приложения | **не опубликовано** |
| **Статус поставщика технологий** | **не получен — без него заявку не подать** |
| Запрос на проверку | **не создан** |
| Скринкасты | **не записаны** |

Про поставщика технологий (Tech Provider): на панели приложения Мета пишет прямо — статус
нужен, «чтобы подать заявку на проверку приложения и запросить доступ к данным пользователей
и других компаний». То есть это шаг ДО подачи, а не после. Ссылка «Стать поставщиком
технологий» — там же на панели.
...
## Две настройки, без которых поток не стартует

«Невозможно загрузить URL. Домен этого URL не включен в список доменов приложения» = не
заполнено одно из двух. Обе исправлены 03.08.2026; при смене домена править снова.

1. **Настройки приложения → Основное → Домены приложений** — должен быть **точный** хост, не
   только корневой домен: `zapleo.com` + `stepan2.zapleo.com`.
2. **Вход через Facebook для бизнеса → Настройки → Действительные URI перенаправления для
   OAuth** — `https://stepan2.zapleo.com/connect/meta/callback`. Было пусто, а «строгий режим
   для URI перенаправления» включён, то есть совпадение требуется полное.

Значение должно **побайтово** совпадать с тем, что строит `_redirect_uri()` в
`app/api/_routes_connect.py`. Проверить, что отдаёт код, можно не открывая браузер:

```bash
curl -s -o /dev/null -w '%{redirect_url}\n' https://stepan2.zapleo.com/connect/meta/16/start
```
