...
- [ ] **Auth включён** (`AUTH_ENABLED=true`) — см. ниже. До включения UI открыт.

## Аутентификация (Telegram Login)

Гейт реализован, но **по умолчанию выключен** (`AUTH_ENABLED=false`), чтобы выкатка
кода не заблокировала вход до настройки бота/домена. Пока выключен — `/ui` и `/admin`
открыты; на это время поставь nginx basic-auth или IP-allowlist перед `:8020`.
