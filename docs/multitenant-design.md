...
Принуждение — два слоя:

1. **`WriteGuardMiddleware`** (грубый, центральный): любой изменяющий `POST /ui/**`
   блокируется (403) для сессии без права записи вообще (`sa=False`, `wr=[]` →
   pure branch_viewer). Так ни один из ~30 write-роутов нельзя забыть закрыть.
   Исключение — «read-support» POST'ы, которые нужны для **чтения** и ничего не
   мутируют (см. `_is_read_support_post`): перевод сообщений/черновика (`/translate`,
   `/tr-draft`, `/tr`), анализ чата (`/analyze/`), догрузка истории (`/load-context`),
   смена view-cookie (`/branch-filter`).

2. **Per-branch в самих write-роутах** (точный): роут резолвит целевой филиал ресурса
   и проверяет его против **writable-набора**, а не readable — write-роуты передают
   `writable_branch_ids(request)` в тот же guard (`is_branch_forbidden` /
   `is_branch_write_forbidden`), где read-роуты передают `allowed_branch_ids`. Это и
   есть поддержка **смешанных ролей**: пользователь-`branch_admin` филиала 1 и
   `branch_viewer` филиала 2 (`br=[1,2]`, `wr=[1]`) **читает оба**, но **пишет только в
   1** — запись в филиал 2 (где он viewer) отклоняется, хотя первый слой его пропускает
   (`wr` непустой).
...
### Meta Page Messenger: список чатов, окно 24ч, видимость отправки

- **Полный список чатов.** `GraphTransportHTTP.fetch_conversations`
  ([transports.py](../app/adapters/channels/transports.py)) идёт по `/{page}/conversations`
  с курсорной пагинацией (`after`) до `meta_live_conversations` (дефолт 100). Дефолтная
  страница Graph — всего ~25, из-за чего старые-но-активные чаты раньше терялись. Auth в
  заголовке (не `?access_token=`) — курсор в URL не светит токен в логах.
- **Гейт окна 24ч.** Meta отклоняет сообщение, отправленное >24ч после последнего входящего
  лида. `OutboxSender.send_next` для `meta_business` **пропускает** обречённый API-вызов
  автоматической (agent/followup) отправки при `window_until < now`: строка → `skipped`
  (`meta_window_closed`), не `failed` — это ожидаемо, не ошибка менеджера; фолоап-цикл
  возобновится, когда лид напишет и ingest переоткроет окно. **Отправка менеджера** всё равно
  пытается (человек-агент может доставить через 7-дневный `human_agent` тег), а результат
  виден ему (см. ниже). Не-Meta каналы на окно не гейтятся.
- **Менеджер видит фейл.** `fetch_pending` ([_query.py](../app/api/_query.py)) отдаёт и
  `failed`-строки (с текстом ошибки), чат рисует красный пузырёк «✗ не отправлено · <причина>»
  с «Повторить» (`/pending/{oid}/retry` — назад в `pending`, due-now, ошибка сброшена) и
  «Убрать» (`/pending/{oid}/delete` — `skipped`). Раньше упавшая отправка молча исчезала:
  `fetch_pending` брал только `pending`, и при переходе в `failed` пузырёк пропадал без следа.
  `skipped` (авто-отправки, гейтнутые окном) остаются скрыты — не шумят менеджеру.
- **Фильтр инбокса по коннектору — серверный.** Чипы (IG/Meta/WhatsApp) у «Входящие»
  перезагружают список тредов через `/ui/threads?kind=…` ([ui.py](../app/api/ui.py)), а не
  прячут строки на клиенте. Список ограничен `LIMIT 100` по свежести, так что клиентский
  фильтр показывал бы только чаты коннектора, попавшие в глобальный топ-100 (баг «2 Meta-чата
  из 81» — их вытесняли более свежие Instagram-треды). Серверный `ch.kind = :kind` тянет
  именно топ-100 нужного коннектора. Активный чип подсвечен, повторный клик — сброс.
...
## Откуда Meta Business канал берёт токен

Токен системного пользователя живёт в **настройке канала** `meta_system_user_token`
(`app_setting(branch_id, channel_id, key)`), туда его пишет редактор коннектора.
`build_channel_port` (`app/worker/wiring.py`) читает именно её.

Так было не всегда, и это стоило одного тихого дня: раньше токен читался **только**
из `channel_session.secret_enc`, а в проекте **нет ни одного места, которое пишет
`ChannelSession` для `meta_business`**. Оператор вставлял верный токен, интерфейс
показывал «сохранено», а воркер на каждом тике писал `no active token for Meta
Business channel N` — и канал не мог заработать в принципе. `ChannelSession` оставлен
как приоритетный fallback на случай, если что-то начнёт его заполнять.

Там же поправлен хост Graph: транспорт ходит в `/{page-id}/conversations` и
`/{page-id}/messages` — это пути Messenger Send API, они живут на
**`graph.facebook.com`**. Дефолт указывал на `graph.instagram.com`, где их нет.

`account_id` (он же Page ID) резолвится по цепочке: сессия → настройка `meta_page_id`
→ `channel.account_id`. Пустой `account_id` не падает, а молча даёт 404 на каждый
запрос, поэтому он закреплён тестом.

## Подключение клиента: Facebook Login for Business (`/connect/meta/...`)

До этого Meta-канал подключался вставкой System User токена в админку. Для нас это работало,
для клиента — нет, и главное: **App Review требует записи экрана, где пользователь сам выдаёт
разрешения**. Поле для вставки токена такого показать не может, поэтому заявка без этого
потока отклоняется автоматически.

Два маршрута, оба публичные (`_PUBLIC_PREFIXES`), потому что Meta возвращает браузер редиректом
**без сессионной куки**:

- `GET /connect/meta/{channel_id}/start` — редирект на экран согласия Meta;
- `GET /connect/meta/callback` — приём кода, обмен на токен, сохранение сессии канала.

**Ничему из запроса не доверяем.** Идентификатор канала едет в параметре `state`, подписанном
HMAC (`app/modules/meta/oauth.py`, переиспользует `sign`/`verify` из `_session.py`), живёт
15 минут. Подделанный, изменённый или просроченный `state` отвергается — иначе кто угодно
подсунул бы свой код и привязал свою страницу к чужому каналу.

Callback сохраняет **Page-токен** (не user-токен: Send API принимает только его) в
`channel_session` ровно в том же виде, что и ручная форма, — воркер разницы не видит.

Запрашиваемые разрешения намеренно минимальны (`SCOPES`): App Review отклоняет права, работу
которых приложение не может показать, а каждый лишний scope — это то, чем клиента просят
рискнуть без нужды.

**Секрет приложения — один на систему.** `app/modules/meta/app_secret.py` — единственное
место, откуда его читают И вебхуки, И этот поток. Порядок: `STEPAN2_META_APP_SECRET_<branch>`
(филиал 1 работает с чужим приложением клиента), затем `STEPAN2_META_APP_SECRET` — приложение
продукта, чтобы новый клиент не требовал новой переменной окружения. Поля в админке нет
сознательно: копия значения — это способ разойтись с оригиналом после ротации.
